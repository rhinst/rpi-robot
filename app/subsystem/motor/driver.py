...
class Driver(Subsystem):
    ...
    def drive(
        ...
        GPIO.output(
            ...
            GPIO.HIGH if front_left.direction == MotorDirection.FORWARD else GPIO.LOW,
        )
        self.fr_pwm.start(front_right.speed * 100)
        GPIO.output(
            self.pins.front_right_dir,
            GPIO.HIGH if front_right.direction == MotorDirection.FORWARD else GPIO.LOW,
        )
        self.rl_pwm.start(rear_left.speed * 100)
        GPIO.output(
            self.pins.rear_left_dir,
            GPIO.HIGH if rear_left.direction == MotorDirection.FORWARD else GPIO.LOW,
        )
        self.rr_pwm.start(rear_right.speed * 100)
        GPIO.output(
            self.pins.rear_right_dir,
            GPIO.HIGH if rear_right.direction == MotorDirection.FORWARD else GPIO.LOW,
        )
